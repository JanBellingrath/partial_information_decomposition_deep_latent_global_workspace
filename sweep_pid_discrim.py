...
def precompute_and_cache_features(data_module, domain_modules, device, batch_size, cache_dir, fusion_model=None):
    ...
    for split in splits:
        ...
        # This function already handles proper tensor formatting for each domain
        if fusion_model is not None:
            ...
            samples = generate_samples_from_dataset_fixed(
                ...
                split=split,
                n_samples=10000,  # We'll trim these later if needed
                batch_size=batch_size,
                device=device,
...
def compute_sweep_clusters(precomputed_data, sweep_id, num_clusters=10, cluster_method='kmeans', cluster_seed=42, normalize_inputs=False):
    """Compute clusters once at the start of a sweep and save them for all trials to use."""
    ...
    if os.path.exists(clusters_path):
        ...
        # Verify the saved config matches current config
        if cluster_data['config'] != {
            ...
            raise ValueError("Saved cluster configuration does not match current configuration!")
        
        # For validation/test predictions, we need to refit the clusterer with the saved centers
        if cluster_method == 'kmeans':
            from sklearn.cluster import KMeans
            clusterer = KMeans(
                n_clusters=num_clusters,
                random_state=cluster_seed,
                n_init=1  # Only one init since we're setting the centers
            )
            # Set the cluster centers and labels directly
            clusterer.cluster_centers_ = cluster_data['cluster_centers']
            clusterer.labels_ = cluster_data['train_labels'].numpy()
        else:  # GMM
            from sklearn.mixture import GaussianMixture
            clusterer = GaussianMixture(
                n_components=num_clusters,
        ...
        return (
            clusterer,
            cluster_data['train_labels'],
            cluster_data['normalization_params'] if normalize_inputs else None
        )
    ...
    if cluster_method == 'kmeans':
        ...
        # Fit and extract necessary data
        clusterer.fit(train_gw.numpy())
        cluster_data = {
            'config': {
                'num_clusters': num_clusters,
                'method': cluster_method,
            ...
            'train_labels': torch.tensor(clusterer.labels_, dtype=torch.long),
    ...
    return clusterer, cluster_data['train_labels'], normalization_params
...
def get_datasets(precomputed_data, num_clusters=10, cluster_method='kmeans', cluster_seed=42, normalize_inputs=False):
    ...
    clusterer, train_labels, normalization_params = compute_sweep_clusters(
    ...
    # Return single datasets for each split
...
def train():
    ...
    try:
        # Initialize wandb with sweep config - let wandb handle the parameters
        wandb.init()
        
        # Get combined config - this will include both sweep parameters and our CLI args
        config = wandb.config
        ...
        if CACHED_FEATURES is None:
            try:
                ...
                CACHED_FEATURES = precompute_and_cache_features(
                    ...
                    cache_dir=cache_dir,
                    fusion_model=fusion_model  # Pass fusion model for GW rep computation
                )
                
            except Exception as e:
                print(f"Error loading cached features: {e}")
                print("Falling back to generating features for each split...")
                
                # Generate features for each split separately
                CACHED_FEATURES = {}
                for split in ['train', 'val', 'test']:
                    print(f"\nGenerating features for {split} split...")
                    # 1) Get raw features from dataset
                    raw_features = generate_samples_from_dataset_fixed(
                        model=fusion_model,
                        data_module=data_module,
                        domain_names=['v_latents', 't'],
                        split=split,
                        n_samples=getattr(config, f"{split}_size", None),
                        batch_size=config.batch_size,
                        device=device,
                        use_gw_encoded=False
                    )

                    # 2) Move features to device and handle projector
                    with torch.no_grad():
                        v_features = raw_features['v_latents'].to(device)
                        t_features = raw_features['t'].to(device)

                        # Apply text projector if present
                        if hasattr(fusion_model.domain_mods['t'], 'projector'):
                            t_features = fusion_model.domain_mods['t'].projector(t_features)

                        # Handle v_latents extra dimensions if present
                        if v_features.dim() > 2:
                            v_features = v_features[:, 0, :]  # Take first component (mean vector)

                        # 3) Compute GW representation using fusion model's fuse method
                        gw_rep = fusion_model.fuse({
                            'v_latents': v_features,
                            't': t_features
                        }, None)  # Selection scores are ignored in ConfigurableFusion

                        # 4) Store all features back on CPU
                        CACHED_FEATURES[split] = {
                            'v_latents': raw_features['v_latents'],  # Keep original v_latents
                            't': raw_features['t'],                  # Keep original t features
                            'gw_rep': gw_rep.cpu()                  # Add computed GW rep
                        }

                        print(f"Cached features for {split} split with shapes:")
                        print(f"  v_latents: {CACHED_FEATURES[split]['v_latents'].shape}")
                        print(f"  t: {CACHED_FEATURES[split]['t'].shape}")
                        print(f"  gw_rep: {CACHED_FEATURES[split]['gw_rep'].shape}")

                # Verify we have all splits
                missing_splits = {'train', 'val', 'test'} - set(CACHED_FEATURES.keys())
                if missing_splits:
                    raise RuntimeError(f"Failed to generate features for splits: {missing_splits}")

                # Verify GW representations are present in all splits
                for split in ['train', 'val', 'test']:
                    if 'gw_rep' not in CACHED_FEATURES[split]:
                        raise RuntimeError(f"GW representation missing from {split} split")
        
        # Verify cached features have all required splits and GW representations
        required_splits = {'train', 'val', 'test'}
        missing_splits = required_splits - set(CACHED_FEATURES.keys())
        if missing_splits:
            raise RuntimeError(f"Cached features missing required splits: {missing_splits}")
        
        for split in required_splits:
            if 'gw_rep' not in CACHED_FEATURES[split]:
                raise RuntimeError(f"GW representation missing from {split} split")

        # Get datasets with cached features
        ...
        def get_dataset_fingerprint(ds):
            """Get a fingerprint of the dataset's content"""
            sample = next(iter(DataLoader(ds, batch_size=1)))
            if isinstance(sample, (list, tuple)):
                tensors = []
                for x in sample:
                    if isinstance(x, torch.Tensor):
                        # Convert Long tensor to float before calculating mean
                        if x.dtype == torch.long or x.dtype == torch.int64:
                            tensors.append(x.float().mean().item())
                        else:
                            tensors.append(x.mean().item())
            else:
                # Handle single tensor case
                if isinstance(sample, torch.Tensor):
                    if sample.dtype == torch.long or sample.dtype == torch.int64:
                        tensors = [sample.float().mean().item()]
                    else:
                        tensors = [sample.mean().item()]
                else:
                    tensors = []
            return tuple(tensors)
        
        train_fp = get_dataset_fingerprint(train_ds)
        val_fp = get_dataset_fingerprint(val_ds)
        test_fp = get_dataset_fingerprint(test_ds)
        
        if train_fp == val_fp or train_fp == test_fp or val_fp == test_fp:
            raise RuntimeError("Detected duplicate datasets between train/val/test splits!")
        
        # Create data loader with pinned memory and non-blocking transfers
        ...
        val_loader = DataLoader(
            val_ds,  # Using validation dataset directly
            batch_size=config.batch_size,
            shuffle=False,
            num_workers=8 if torch.cuda.is_available() else 0,
            pin_memory=True,
        ...
        test_loader = DataLoader(
            test_ds,  # Using test dataset directly
            batch_size=config.batch_size,
            shuffle=False,
            num_workers=8 if torch.cuda.is_available() else 0,
            pin_memory=True,
        ...
        # Create discriminators with dropout
        ...
        # Log best config and test metrics
