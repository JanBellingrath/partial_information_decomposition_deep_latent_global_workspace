...
def generate_boolean_data(
    ...
    Args:
        ...
        add_noise: Whether to add Gaussian noise to make discrimination harder
    ...
    # Add noise to inputs to make discrimination more challenging
    if add_noise:
        input_a = input_a + torch.randn_like(input_a) * noise_std
        input_b = input_b + torch.randn_like(input_b) * noise_std
    ...
    for func_name in functions:
        ...
        # Add noise to outputs as well
        if add_noise:
            output = output + torch.randn_like(output) * noise_std * 0.5  # Less noise for outputs
